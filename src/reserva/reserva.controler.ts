...
async function findAll(req: Request, res: Response) {
  try{
    const reservas = await em.find(Reserva, {}, {populate: ['turno', 'cancha']});
...
async function findOne(req: Request, res: Response) {
  try{
    ...
    const reserva = await em.findOneOrFail(Reserva, {id}, {populate: ['turno', 'cancha']}); // el 2do parametro es un filtro
