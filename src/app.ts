...
app.post('/api/canchas', sanitizedCanchaInput,(req, res) => {
  ...
  canchas.push(cancha);
  res.status(201).send({message: 'Cancha creada', data:cancha}) //el status indica que el recurso se creo correctamente, mas adelante se detallan los dif tipos de status
});

app.put('/api/canchas/:id',sanitizedCanchaInput, (req, res) => {
    ...
    if(canchaIdx===-1){
        res.status(404).send({menssage: 'Cancha no encontrada'})
    }

    canchas[canchaIdx] = {...canchas[canchaIdx], ...req.body.sanitizedInput} //generamos la modificacion de los datos sobre el objeto

    res.status(200).send({menssage: 'La cancha se actualizo correctamente', data:canchas[canchaIdx]}) 
})
